...
class pdf_roles extends FPDF{
	   ...
	   public function Encabezado_Tabla(){
		...
		$this -> setfillcolor(255, 255, 255);
		 ...
		 $this -> cell(60,10,"rol",0,0,'C','true');
		$this -> cell(60,10,"Tipo rol",0,0,'C','true');
		$this -> cell(60,10,"Estado",0,0,'C','true');
		$this-> SetDrawColor(51, 181, 255);
		$this-> SetLineWidth(1);
	   ...
	   public function Cuerpo_Tabla($pdo){
		...
		$this->SetDrawColor(16, 15, 15);
	   	$stm = $pdo -> Query("SELECT r.Id_rol, r.Tipo_rol, e.Nombre FROM rol r
		   NATURAL JOIN estado e");
	   	while ($data = $stm -> fetch(PDO::FETCH_OBJ))
	   	{
			$this -> Ln(10);
			$this -> cell(60,10,$data->Id_rol,1,0,'L');
			$this -> cell(60,10,$data->Tipo_rol,1,0,'L');
