...
class pdf_sede extends FPDF{
	...
	$this -> setfillcolor(255, 255, 255);
	 ...
	 $this -> cell(60,10,"Sede",0,0,'C','true');
	$this -> cell(60,10,"Tipo sede",0,0,'C','true');
	$this -> cell(60,10,"Direccion",0,0,'C','true');
	$this-> SetDrawColor(51, 181, 255);
	$this-> SetLineWidth(1);
	...
	$this -> Ln(2);
	   ...
	   public function Cuerpo_Tabla($pdo){
		...
		$this->SetDrawColor(16, 15, 15);
	   	$stm = $pdo -> Query("SELECT * FROM sede");
	   	while ($data = $stm -> fetch(PDO::FETCH_OBJ))
	   	{
			$this -> Ln(10);
			$this -> cell(60,10,$data->Id_sede,1,0,'c');
			$this -> cell(60,10,$data->Tipo_sede,1,0,'c');
			$this -> cell(70,10,$data->Direccion,1,0,'L');
