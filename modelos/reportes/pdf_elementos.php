...
class pdf_elementos extends FPDF{
	...
	$this -> cell(50,10,"Nombre",0,0,'C','true');
	$this -> cell(50,10,"Descripcion",0,0,'C','true');
	$this -> cell(50,10,"Referencia",0,0,'C','true');
	$this -> cell(50,10,"Aula",0,0,'C','true');
	$this-> SetDrawColor(51, 181, 255);
	$this-> SetLineWidth(1);
	...
	$this -> Ln(2);
	   ...
	   public function Cuerpo_Tabla($pdo){
		...
		$this->SetDrawColor(16, 15, 15);
	   	$stm = $pdo -> Query("SELECT * FROM elemento");
	   	while ($data = $stm -> fetch(PDO::FETCH_OBJ))
	   	{
			$this -> Ln(10);
			$this -> cell(50,10,$data->Nombre_elemento,1,0,'c');
			$this -> cell(50,10,$data->Descripcion_elemento,1,0,'c');
			$this -> cell(50,10,$data->Referencia,1,0,'c');
			$this -> cell(45,10,$data->Id_aula,1,0,'c');
